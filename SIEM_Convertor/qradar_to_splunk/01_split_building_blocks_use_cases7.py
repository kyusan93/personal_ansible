...
BUILDING_BLOCK_PATTERN = r'BB:[^,\n]+'

def extract_building_blocks(text):
    matches = re.findall(BUILDING_BLOCK_PATTERN, text)
    return matches

def extract_usingbb_tests(text, data_dicts):
    usingbb_tests = []
    for other_dict in data_dicts:
        if (
            text["UsingBB"] in other_dict["Rule Name"]
            and other_dict["Tests"] == "TRUE"
        ):
            usingbb_tests.append(other_dict["Tests"])
    return "\n".join(usingbb_tests)
...
def process_data(file_path):
    ...
    with open(file_path, newline="") as csvfile:
        ...
        for row in reader:
            row["UsingBB"] = "\n".join(list(extract_building_blocks(row["Tests"])))
            ...
            row["UsingBB_Tests"] = extract_usingbb_tests(row, data_dicts)
            row["Check Usage"] = ""
            ...
            if not row["Used by Building Blocks"] and not row["Used by Use Cases"]:
                row["Check Usage"] = "Not Used"
                not_used_tests_lines = tests_lines
            else:
                row["Check Usage"] = "Used"

            # Extract Not Used Test Lines
            if not_used_tests_lines:
                row["Not Used Tests"] = "\n".join(not_used_tests_lines)

            # Append row to the list of processed data
